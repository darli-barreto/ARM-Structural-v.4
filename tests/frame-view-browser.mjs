...
try{
  ...
  await page.waitForFunction(()=>document.querySelector('#model-toolbar')?.dataset.modelStatus?.includes('Uniones listas'));
  await page.locator('#model-analytical').click();await settle();
  assert.equal(await page.locator('#model-slab-contours').isChecked(),false);assert.equal(await page.locator('#model-toolbar').getAttribute('data-slab-contours'),'0');
  await page.locator('#model-frame-plane').selectOption('XY');await page.waitForFunction(()=>document.querySelector('#model-toolbar')?.dataset.axisElements==='88');
  assert.equal(await page.locator('#model-projection').inputValue(),'elevation');await snapshot('xy-clean');
  await page.locator('#model-frame-plane').selectOption('all');await page.locator('#model-slab-contours').check();await page.waitForFunction(()=>Number(document.querySelector('#model-toolbar')?.dataset.slabContours)>0);
  ...
  assert.match(await page.locator('#model-toolbar').getAttribute('data-model-status'),/ZY \/ X = 0/);
  await page.locator('#model-frame-ordinate').fill('100');await page.locator('#model-frame-ordinate').press('Tab');await page.waitForFunction(()=>document.querySelector('#model-toolbar')?.dataset.modelStatus?.includes('Sin barras en este plano'));
  ...
  await page.locator('#model-diagram').selectOption('moment');await page.waitForFunction(()=>document.querySelector('#model-toolbar')?.dataset.modelStatus?.includes('|max|'));await snapshot('calculation-moment');
  ...
  await page.locator('#model-scope').selectOption('calculation');await page.locator('#model-diagram').selectOption('moment');await page.waitForFunction(()=>document.querySelector('#model-toolbar')?.dataset.modelStatus?.includes('|max|'));
