...
try{
  ...
  await page.waitForFunction(()=>document.querySelector('#model-toolbar')?.dataset.modelStatus?.includes('Neto 2268.866'),{},{timeout:60000});
  ...
  await page.locator('#model-diagram').selectOption('moment');await page.waitForFunction(()=>document.querySelector('#model-toolbar')?.dataset.modelStatus?.includes('|max|'));
