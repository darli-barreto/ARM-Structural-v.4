...
try{
  ...
  const bounds=await page.locator('#examples-modal > section').boundingBox();assert.ok(bounds.x>=0&&bounds.x+bounds.width<=390);
  ...
  await page.locator('#ribbon-tab-analysis').click();await page.locator('#project-analysis').click();assert.equal(await page.locator('#analysis-tolerance').inputValue(),'0.001');
  ...
  await page.locator('#ribbon-tab-structure').click();await page.locator('#btn-full-building').click();await page.locator('input[value="cantilever"]').check();page.once('dialog',d=>d.dismiss());await page.locator('#examples-load').click();
  assert.equal(await page.locator('#examples-modal').isVisible(),true);await page.locator('#examples-close').click();
  await page.locator('#ribbon-tab-analysis').click();await page.locator('#project-analysis').click();await page.locator('[data-tab="nodes"]').click();assert.equal(await page.locator('[data-support]').count(),103);await page.locator('#analysis-close').click();
  for(const id of ['cantilever','supported']){
    await choose(id,true);assert.match(await page.locator('#analysis-table').textContent(),/Sin resultados vigentes/);
    ...
    assert.deepEqual(await page.locator('.benchmark-comparison tbody tr td:last-child').allTextContents(),['Coincide','Coincide','Coincide']);
  ...
  await page.waitForFunction(()=>document.querySelector('#application-title-bar')?.textContent?.includes('Guardado local'));
  await page.reload({waitUntil:'domcontentloaded'});await page.waitForFunction(()=>document.querySelector('#application-title-bar')?.textContent?.includes('Proyecto recuperado'));
  await page.locator('#ribbon-tab-analysis').click();await page.locator('#project-analysis').click();await solve();assert.equal(await page.locator('.benchmark-comparison h3').textContent(),'Coincide con la referencia');
  await page.locator('[data-tab="members"]').click();await page.locator('[data-key="dead"]').fill('11');await page.locator('[data-key="dead"]').press('Tab');
  await page.waitForFunction(()=>document.querySelector('#analysis-status')?.textContent?.includes('Entradas modificadas'));
  await solve();
  assert.equal(await page.locator('.benchmark-comparison').count(),0);assert.match(await page.locator('.benchmark-note').textContent(),/Modelo modificado/);
