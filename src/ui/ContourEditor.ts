...
export class ContourEditor {
  ...
  constructor(private commit:(el:ManagedElement,definition:StructuralDefinition)=>void) {
    this.overlay.className='workspace-overlay';this.overlay.id='contour-editor';this.overlay.hidden=true;
  ...
  public open(el:ManagedElement,addHole=false):void {
    ...
    this.overlay.hidden=false;this.render();this.fit();this.paint();this.get<HTMLButtonElement>('contour-finish').focus();
  }
  private close():void {this.overlay.hidden=true;this.observer.disconnect();this.previous?.focus();}
  ...
  private render():void {
    ...
    this.overlay.innerHTML=`<section class="workspace-dialog" style="max-width:1150px"><header class="workspace-heading"><div><span class="workspace-eyebrow">MODIFICAR / ${h(this.element.bimDoc?.instanceParameters.mark||this.element.id)}</span><h2>${slab?'Editar contorno':'Editar geometria'}</h2></div><div class="workspace-actions"><button id="contour-cancel">${icon('X')} Cancelar</button><button id="contour-finish" class="primary">${icon('Check')} Finalizar</button></div></header>
    <div class="sketch-toolbar"><button id="contour-undo" class="icon-button" title="Deshacer" aria-label="Deshacer">${icon('Undo2')}</button><button id="contour-redo" class="icon-button" title="Rehacer" aria-label="Rehacer">${icon('Redo2')}</button><button id="contour-fit" class="icon-button" title="Encuadrar" aria-label="Encuadrar">${icon('Crosshair')}</button>${slab?`<button id="contour-draw">${icon('Pencil')} Dibujar contorno</button><button id="contour-hole">${icon('Plus')} Dibujar hueco</button><button id="contour-close-ring">${icon('Check')} Cerrar trazo</button><button id="contour-discard-ring">${icon('X')} Descartar trazo</button>`:''}</div>
    <div class="sketch-layout"><div class="sketch-canvas-wrap"><canvas id="contour-canvas" aria-label="Vista de geometria editable"></canvas></div><aside class="sketch-sidebar"><div id="contour-params"></div><div id="contour-rings"></div><table class="sketch-vertices"><thead><tr><th>#</th><th>X (m)</th><th>${slab?'Z (m)':'Y (m)'}</th>${slab?'':'<th>Z (m)</th>'}<th></th></tr></thead><tbody id="contour-vertices"></tbody></table><div id="contour-error" class="workspace-error" role="alert"></div></aside></div>
    <footer class="workspace-footer"><span id="contour-status"></span><span id="contour-quantities"></span></footer></section>`;
    ...
    if(slab){
      const move=document.createElement('button');move.id='contour-move';move.className='icon-button';move.title='Mover elemento completo';move.setAttribute('aria-label',move.title);move.innerHTML=icon('Move');move.setAttribute('aria-pressed','false');
      move.onclick=()=>{this.moving=!this.moving;this.drawing=null;this.pending=[];this.preview=null;this.guide=null;move.classList.toggle('primary',this.moving);move.setAttribute('aria-pressed',String(this.moving));this.refresh();};
      const ortho=document.createElement('label');ortho.style.cssText='flex-direction:row;align-items:center;gap:6px';ortho.title='Restringir a un eje (Ctrl o Shift)';ortho.innerHTML=`<input id="contour-ortho" type="checkbox" ${this.ortho?'checked':''}/> Ortogonal`;
      ortho.querySelector('input')!.onchange=e=>{this.ortho=(e.target as HTMLInputElement).checked;this.constraint.reset();};
      this.overlay.querySelector('.sketch-toolbar')!.append(move,ortho);
    }
  ...
  private startDrawing(mode:'boundary'|'hole'):void {
    this.moving=false;const b=this.get('contour-move');b.classList.remove('primary');b.setAttribute('aria-pressed','false');
  ...
  private refresh():void {
    ...
    this.get('contour-params').innerHTML=`<div class="sketch-param-grid">${params.map(k=>`<label>${labels[k]}<input type="number" step="0.01" data-param="${k}" value="${Number((d as unknown as Record<string,number>)[k].toFixed(6))}"/></label>`).join('')}</div>`;
    ...
    if(d.type==='slab'){
      this.get('contour-rings').innerHTML=`<label>Contorno<select id="contour-ring"><option value="0">Exterior</option>${(d.voids||[]).map((_,i)=>`<option value="${i+1}">Hueco ${i+1}</option>`).join('')}</select></label><div class="workspace-actions" style="margin-bottom:12px"><button id="contour-insert" title="Insertar vertice despues del seleccionado">${icon('Plus')} Vertice</button><button id="contour-remove-hole" ${this.ring===0?'disabled':''}>${icon('Trash2')} Hueco</button></div>`;
    ...
    this.get('contour-vertices').innerHTML=this.points().map((p,i)=>`<tr class="${i===this.vertex?'active':''}"><td>${i+1}</td>${axes.map(a=>`<td><input aria-label="Vertice ${i+1} ${a.toUpperCase()}" type="number" step="0.01" data-vertex="${i}" data-axis="${a}" value="${Number(p[a].toFixed(4))}" /></td>`).join('')}<td>${d.type==='slab'?`<button data-remove="${i}" class="icon-button" title="Eliminar vertice ${i+1}" aria-label="Eliminar vertice ${i+1}" ${this.points().length<=3?'disabled':''}>${icon('Minus')}</button>`:''}</td></tr>`).join('');
